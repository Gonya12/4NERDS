...
export function CardScanPanel({ imageFile, backImageFile, category, inventory, onApply }: Props) {
  ...
  async function scan(force = false) {
    ...
    try {
      ...
      setMessage(result.cached ? "Loaded cached scan. Review every field before applying." : "Scan complete. Review every field before applying.");
    } catch (error) {
  ...
  return <section className="space-y-3 rounded-2xl border border-violet-200 bg-violet-50/70 p-3 dark:border-violet-900 dark:bg-violet-950/20">
    ...
    {suggestion ? <div className="space-y-3"><div className="grid gap-2 sm:grid-cols-3">{field("cardName", "Card name")}{field("collectorNumber", "Collector number")}{field("cardSet", "Set / code")}{field("language", "Language")}{field("condition", "Condition")}{field("stickerPrice", "Sticker / asking price", "number")}{category === "graded_card" ? <>{field("gradingCompany", "Grading company")}{field("grade", "Grade")}{field("certificateNumber", "Certificate number")}</> : null}</div>{duplicateCertificate ? <p className="rounded-xl bg-rose-100 p-2 text-sm font-black text-rose-800">Possible duplicate slab certificate.</p> : null}<button type="button" onClick={() => { onApply(suggestion, hash); setMessage("Suggestions applied. Confirm the normal inventory form, then Save."); }} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white"><Sparkles size={17} />Apply Suggestions</button></div> : null}
  </section>;
}
