...
export function TransactionItemPricing({ item, context, onChange }: Props) {
  ...
  return <div className="space-y-3">
    ...
    {item.itemType === "raw_card" && context === "purchase" ? <TargetPriceCalculator marketValue={item.marketValue} percentage={percentage} onPercentage={choosePercentage} actionLabel="Use as Cash Paid" onApply={(amount) => onChange({ ...item, targetBuyPercentage: percentage, targetBuyPrice: amount, boughtPrice: amount })} note="Choose a target first. Cash Paid changes only when you tap Use as Cash Paid; Cost Basis remains separate." /> : null}
    ...
    {context === "sale" ? <div className="grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-3 text-xs dark:bg-slate-900"><span>Original Cost Basis<br /><b>{basisKnown ? formatMoney(item.historicalCostBasis) : "Cost basis required"}</b></span><span>Sale Profit<br /><b className={grossProfit == null ? "text-amber-700" : grossProfit >= 0 ? "text-emerald-600" : "text-rose-600"}>{grossProfit == null ? "Pending cost basis" : formatMoney(grossProfit)}</b></span>{item.costBasisIsEstimate ? <span className="col-span-2 font-bold text-amber-700">Estimated cost basis — not linked to inventory history.</span> : null}</div> : null}
  </div>;
}
