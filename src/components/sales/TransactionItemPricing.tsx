...
export function TransactionItemPricing({ item, context, onChange }: Props) {
  ...
  const grossProfit = Number(item.soldPrice || 0) - item.historicalCostBasis;
  ...
  return <div className="space-y-3">
    ...
    {context === "sale" ? <div className="grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-3 text-xs dark:bg-slate-900"><span>Original Cost Basis<br /><b>{formatMoney(item.historicalCostBasis)}</b></span><span>Gross Profit<br /><b className={grossProfit >= 0 ? "text-emerald-600" : "text-rose-600"}>{formatMoney(grossProfit)}</b></span>{item.costBasisIsEstimate ? <span className="col-span-2 font-bold text-amber-700">Estimated cost basis — not linked to inventory history.</span> : null}</div> : null}
  </div>;
}
