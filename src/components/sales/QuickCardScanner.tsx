  ...
  return <div className="fixed inset-0 z-[100] overflow-hidden bg-black text-white">
    {mode === "camera" ? <>
      ...
      <main className="mx-auto mt-4 max-w-2xl space-y-4">
        ...
        {suggestion && hasUsefulSuggestion ? <section className="space-y-3 rounded-2xl bg-white p-4 text-slate-950">
          {suggestion.correctedNameCandidate ? <div className="rounded-xl border border-violet-200 bg-violet-50 p-3"><p className="text-xs font-bold text-slate-500">Possible detected card</p><p className="font-black">{suggestion.correctedNameCandidate}</p><p className="text-xs capitalize text-violet-700">{suggestion.correctedNameConfidence || "low"} confidence</p></div> : null}
