...
export function IncomingBatchPricing({ items, mode, onApply }: Props) {
  ...
  return <section className="surface-card space-y-3 p-4">
    <div><h3 className="font-black">Batch incoming pricing</h3><p className="text-xs text-slate-500">Outgoing inventory is never changed. {mode === "purchase" ? "This sets target prices; Actual Bought Price still requires a deliberate item action." : "This sets each incoming Accepted Trade Value."}</p></div>
