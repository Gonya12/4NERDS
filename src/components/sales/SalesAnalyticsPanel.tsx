  ...
  return (
    <div className="space-y-4">
      ...
      <section onClickCapture={(event) => {
        ...
        {recentRecords.some((row) => row.image) ? <div className="flex gap-2 overflow-x-auto pb-1">{recentRecords.filter((row) => row.image).map((row) => {
          const title = row.type === "sale" ? row.sale.itemName || "Sale details pending" : row.type === "purchase" ? row.purchase.itemName : row.expense.description || "Expense receipt";
          return <button key={`photo-${row.id}`} type="button" className="shrink-0 rounded-xl"><img src={row.image} alt={title} loading="lazy" className="size-20 rounded-xl bg-slate-100 object-contain dark:bg-slate-900" /></button>;
        })}</div> : null}
