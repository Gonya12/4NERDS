...
function WorkerModal({
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end bg-slate-950/50 p-4 backdrop-blur-sm">
      <section className="mx-auto w-full max-w-md rounded-3xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-sm font-bold text-coral">Availability</p>
            <h2 className="text-2xl font-black text-ink">Who can work this event?</h2>
          </div>
          <button onClick={onClose} className="rounded-full bg-slate-100 p-2"><X size={18} /></button>
        </div>
          ...
          {workers.filter((worker) => worker.active).map((worker) => {
            ...
            return (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-bold transition ${active ? "bg-ink text-white shadow-soft" : "bg-slate-100 text-slate-700"}`}
              >
                {worker.name}
              </button>
            );
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button onClick={onClose} className="min-h-12 rounded-xl bg-slate-100 font-bold text-slate-700">Cancel</button>
      ...
      <section className="mx-auto w-full max-w-md rounded-3xl bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="text-2xl font-black text-ink">{payment ? "Edit Payment" : "Add Payment"}</h2>
          </div>
          <button onClick={onClose} className="rounded-full bg-slate-100 p-2"><X size={18} /></button>
        </div>
...
export function EventDetailPage() {
  ...
  if (!event) return <div className="text-sm text-slate-500">Loading event...</div>;
  ...
  return (
    <div className="space-y-5">
      <header className="rounded-3xl bg-ink p-5 text-white shadow-soft">
      ...
      <section className="flex flex-wrap gap-2 rounded-2xl bg-white/90 p-4 shadow-soft">
        <StatusChip value={event.registrationStatus} />
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">{eventTimingStatus(event.startDate)}</span>
      </section>

      <section className="rounded-2xl bg-white/90 p-4 shadow-soft">
        <h2 className="flex items-center gap-2 font-black text-ink"><Users size={18} /> Confirmed Workers</h2>
        <p className={`mt-2 text-sm ${confirmed.length ? "text-slate-700" : "text-slate-400"}`}>{confirmed.length ? confirmed.map((worker) => worker.name).join(", ") : "Nobody confirmed yet"}</p>
      ...
      <section className="space-y-3 rounded-2xl bg-white/90 p-4 text-sm text-slate-700 shadow-soft">
      ...
      <section className="space-y-4 rounded-2xl bg-white/90 p-4 shadow-soft">
        <div className="flex items-center justify-between gap-3">
          <h2 className="flex items-center gap-2 font-black text-ink"><DollarSign size={18} /> Payment Split</h2>
          <button onClick={() => setEditingPayment("new")} className="inline-flex items-center gap-1 rounded-xl bg-ink px-3 py-2 text-xs font-bold text-white"><Plus size={14} /> Add Payment</button>
        </div>
        {paymentSummary.confirmedWorkerCount === 0 ? (
          <p className="rounded-xl bg-slate-50 p-3 text-sm text-slate-500">Add confirmed workers to calculate split.</p>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div className="rounded-xl bg-slate-50 p-3"><p className="text-slate-500">Total cost</p><p className="font-black text-ink">{formatMoney(paymentSummary.totalCost)}</p></div>
              <div className="rounded-xl bg-slate-50 p-3"><p className="text-slate-500">Equal share</p><p className="font-black text-ink">{formatMoney(paymentSummary.equalSharePerWorker)}</p></div>
              <div className="rounded-xl bg-emerald-50 p-3"><p className="text-slate-500">Total paid</p><p className="font-black text-emerald-700">{formatMoney(paymentSummary.totalPaid)}</p></div>
              <div className="rounded-xl bg-amber-50 p-3"><p className="text-slate-500">Remaining</p><p className="font-black text-amber-700">{formatMoney(Math.max(paymentSummary.totalRemaining, 0))}</p></div>
            </div>
            ...
            <div className="space-y-2">
              {paymentSummary.perWorkerSummary.map((worker) => (
                <div key={worker.workerId} className="rounded-xl border border-slate-100 bg-white p-3 text-sm">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-black text-ink">{worker.workerName}</p>
                  ...
                  <p className="mt-2 text-slate-600">Expected {formatMoney(worker.expectedShare)} · Paid {formatMoney(worker.amountPaid)} · {worker.balance > 0 ? `Owes ${formatMoney(worker.balance)}` : worker.balance < 0 ? `Overpaid ${formatMoney(Math.abs(worker.balance))}` : "Paid up"}</p>
                  <p className="mt-1 text-xs text-slate-500">Covered {worker.percentOfTotalPaid.toFixed(2)}% of total cost</p>
                </div>
              ))}
            </div>
          </>
        ...
        {(event.paymentRecords || []).length > 0 ? (
          <div className="space-y-2">
            <h3 className="text-sm font-black text-ink">Payment Records</h3>
            {(event.paymentRecords || []).map((record) => (
              <div key={record.id} className="flex items-center justify-between gap-2 rounded-xl bg-slate-50 p-3 text-sm">
                <div>
                  <p className="font-bold text-ink">{workers.find((worker) => worker.id === record.workerId)?.name || "Unknown"} · {formatMoney(record.amountPaid)}</p>
                  <p className="text-xs text-slate-500">{record.note || "No note"}</p>
                </div>
                <div className="flex gap-1">
                  <button onClick={() => setEditingPayment(record)} className="rounded-lg bg-white px-3 py-2 text-xs font-bold">Edit</button>
      ...
      <section className="grid grid-cols-2 gap-2">
        {event.registrationUrl ? <a href={event.registrationUrl} target="_blank" className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-white text-sm font-bold shadow-soft"><CalendarCheck size={16} /> Register</a> : null}
        {event.sourceUrl ? <a href={event.sourceUrl} target="_blank" className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-white text-sm font-bold shadow-soft"><ExternalLink size={16} /> Source</a> : null}
        {destination ? <a href={googleMapsDirectionsLink(destination)} target="_blank" className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-white text-sm font-bold shadow-soft"><Map size={16} /> Map</a> : null}
        <Link to={`/events/${event.id}/edit`} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl bg-white text-sm font-bold shadow-soft"><Edit size={16} /> Edit</Link>
