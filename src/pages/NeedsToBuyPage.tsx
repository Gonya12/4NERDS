import { ExternalLink, Package, Plus, Search, Trash2, X } from "lucide-react";
...
export function NeedsToBuyPage() {
  ...
  async function saveDraft() {
    ...
    try {
      ...
      await saveBuyItem({
        ...
        purchasedBy: draft.purchasedBy.trim() || undefined,
  ...
  async function togglePurchased(item: BuyItem) {
    await saveBuyItem({
      ...item,
      purchased: !item.purchased,
      purchasedAt: !item.purchased ? nowIso() : undefined,
      updatedAt: nowIso()
    });
    await load();
  }
  ...
  return (
    <div className="space-y-5 lg:mx-auto lg:max-w-7xl">
      ...
      <section className="grid gap-3 rounded-2xl bg-white/90 p-3 shadow-soft md:grid-cols-[1fr_180px_160px] dark:bg-slate-900">
        ...
        {filtered.map((item) => (
          <article key={item.id} onClick={() => openForm(item)} className={`cursor-pointer overflow-hidden rounded-2xl border p-3 shadow-soft transition hover:-translate-y-0.5 ${item.purchased ? "border-emerald-200 bg-emerald-50 dark:border-emerald-900/60 dark:bg-emerald-950/25" : "border-white/70 bg-white/90 dark:border-slate-800 dark:bg-slate-900"}`}>
            <div className="relative">
              ...
              <button onClick={(event) => { event.stopPropagation(); void togglePurchased(item); }} className={`absolute right-2 top-2 rounded-full px-3 py-1 text-xs font-black ${item.purchased ? "bg-emerald-600 text-white" : "bg-white text-slate-700 shadow-soft dark:bg-slate-800 dark:text-white"}`}>
          ...
          <section className="mx-auto max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl bg-white p-5 shadow-2xl dark:bg-slate-900">
            ...
            <div className="mt-4 space-y-3">
              ...
              <label className="flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-sm font-bold dark:bg-slate-950/70">
                <input type="checkbox" checked={draft.purchased} onChange={(event) => setDraft({ ...draft, purchased: event.target.checked })} />
                Purchased
              </label>
            </div>
