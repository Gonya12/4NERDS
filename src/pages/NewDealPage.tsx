...
function paymentMethodsLabel(payments: TransactionPaymentEntry[], method: SalePaymentMethod) {
  const total = roundMoney(payments.filter((payment) => payment.paymentMethod === method).reduce((sum, payment) => sum + Number(payment.amount || 0), 0));
  return `${paymentMethodLabels[method]} ${formatMoney(total)}`;
}
...
export function NewDealPage() {
  ...
  const [selectedSide, setSelectedSide] = useState<DealSide>("incoming");
    ...
    try {
      while (true) {
        ...
        try {
          ...
          const result = await scanPokemonCard(queued.file, "raw_card", undefined, queued.forceRecognition === true, {
            game: "pokemon",
            language: "en",
          ...
          replaceScanQueue((current) => current.map((row) => row.id === queued.id ? { ...row, status, forceRecognition: false, stage: status === "ready" ? "Ready" : "Review required", suggestion: draftSuggestion } : row));
        } catch (unknownError) {
  ...
  const selectedSideItems = selectedSide === "incoming" ? incoming : outgoing;
  const eligiblePercentageItems = selectedSideItems.filter((item) => item.itemType === "raw_card" || item.itemType === "graded_card");
  const detailTitle = transaction.tradePartner || selectedEvent?.name || "Deal details";
  const detailDate = new Date(transaction.tradeDate);
  const detailSummary = `${detailTitle} · ${Number.isNaN(detailDate.getTime()) ? transaction.tradeDate.slice(0, 16) : detailDate.toLocaleString([], { month: "short", day: "numeric", hour: "numeric", minute: "2-digit" })}`;
  ...
  function applyPercentageToSide(percentage: number) {
    updateTransaction({
      ...transaction,
      items: transaction.items.map((item) => item.direction === selectedSide && (item.itemType === "raw_card" || item.itemType === "graded_card")
        ? applyDealPercentage(item, selectedSide, percentage)
        : item),
    });
  }
  ...
  function renderItems(side: DealSide) {
    ...
    if (!rows.length) return <div className="rounded-xl border border-dashed border-slate-300 px-3 py-4 text-center dark:border-slate-700"><p className="text-sm font-black text-slate-600 dark:text-slate-300">No items yet</p><p className="mt-0.5 text-[11px] text-slate-500">Scan a card or choose an add method.</p></div>;
    return <div className="space-y-2">{rows.map((item) => {
      ...
      return <article key={item.id} className="group grid grid-cols-[3rem_minmax(0,1fr)_2.25rem] items-center gap-2.5 rounded-xl border border-slate-200 bg-white p-2.5 shadow-sm transition hover:border-violet-300 dark:border-slate-800 dark:bg-night-850">
        {itemImage(item) ? <img src={itemImage(item)} alt="" className="h-16 w-12 rounded-lg bg-slate-100 object-contain" /> : <div className="grid h-16 w-12 place-items-center rounded-lg bg-slate-100 text-slate-400 dark:bg-slate-900"><Package size={18} /></div>}
        <button type="button" onClick={() => setEditing(item)} className="min-w-0 text-left">
          <span className="flex min-w-0 items-center gap-2"><b className="min-w-0 flex-1 truncate text-sm text-slate-950 dark:text-white">{item.itemName || "Details pending"}</b><small className="shrink-0 rounded-md bg-slate-100 px-1.5 py-0.5 font-black text-slate-600 dark:bg-slate-800 dark:text-slate-300">{item.cardCondition?.match(/\b(NM|LP|MP|HP)\b/)?.[1] || (item.cardCondition === "Damaged" ? "DMG" : "—")}</small></span>
          <span className="mt-0.5 block truncate text-[11px] text-slate-500">{[item.cardSet, item.cardCode || item.collectorNumber].filter(Boolean).join(" · ") || pokemonCategoryLabels[item.itemType]}</span>
          <span className="mt-1 block text-xs text-slate-500">Market <b className="text-slate-800 dark:text-slate-200">{formatMoney(item.marketValue)}</b> · Offer <b className="text-violet-700 dark:text-violet-300">{formatMoney(agreed)}</b> · Qty {item.quantity}</span>
        </button>
        <button type="button" onClick={() => setEditing(item)} aria-label={`Edit ${item.itemName || "item"}`} className="grid size-9 place-items-center rounded-lg text-lg font-black text-slate-400 hover:bg-violet-50 hover:text-violet-700">⋮</button>
      </article>;
    })}</div>;
  }
  ...
  function sidePanel(side: DealSide) {
    ...
    return <section className={`rounded-2xl border p-3 shadow-sm sm:p-4 ${incomingSide ? "border-sky-200 bg-gradient-to-b from-sky-50 to-white dark:border-sky-900/70 dark:from-sky-950/20 dark:to-night-900" : "border-orange-200 bg-gradient-to-b from-orange-50 to-white dark:border-orange-900/70 dark:from-orange-950/20 dark:to-night-900"}`}>
      ...
      <div className="mt-2 grid grid-cols-4 gap-1.5 sm:grid-cols-7">
        ...
        <button type="button" className="deal-add-button" onClick={() => { addPayment(cashDirection); setPaymentsOpen(true); }}><Banknote size={17} />Cash</button>
      </div>
      <div className="mt-2.5">{renderItems(side)}</div>
      {cashTotal > 0 ? <div className="mt-2 flex items-center justify-between rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs dark:border-emerald-900 dark:bg-emerald-950/30"><span className="font-bold">Cash {incomingSide ? "received" : "paid"}</span><b>{formatMoney(cashTotal)}</b></div> : null}
    </section>;
  }
  ...
  const compactMetricLabel = classification === "purchase" ? "Potential inventory gain" : classification === "sale" ? "Estimated profit" : classification === "trade" || classification === "cash_trade" ? "Trade gain/loss" : "";
  ...
  return <div className="mx-auto min-w-0 max-w-7xl space-y-3 pb-28">
    ...
    {step === "build" ? <div className="grid items-start gap-3 lg:grid-cols-[minmax(0,1fr)_20rem]">
      <main className="space-y-2.5">
        <div className="sticky top-0 z-20 space-y-2 rounded-2xl border border-slate-200 bg-canvas/95 p-2 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-950/95 lg:static lg:bg-transparent lg:p-0 lg:shadow-none">
          <div className="grid grid-cols-2 rounded-xl bg-slate-200 p-1 dark:bg-slate-800 lg:hidden" role="tablist" aria-label="Deal side"><button type="button" role="tab" aria-selected={selectedSide === "incoming"} onClick={() => setSelectedSide("incoming")} className={`min-h-11 rounded-lg px-2 text-left text-xs font-black transition ${selectedSide === "incoming" ? "bg-white text-sky-700 shadow-sm dark:bg-slate-950 dark:text-sky-300" : "text-slate-600 dark:text-slate-300"}`}><span className="block">↓ COMING IN</span><small>{incoming.length} · {formatMoney(summary.incomingAgreed)}</small></button><button type="button" role="tab" aria-selected={selectedSide === "outgoing"} onClick={() => setSelectedSide("outgoing")} className={`min-h-11 rounded-lg px-2 text-left text-xs font-black transition ${selectedSide === "outgoing" ? "bg-white text-orange-700 shadow-sm dark:bg-slate-950 dark:text-orange-300" : "text-slate-600 dark:text-slate-300"}`}><span className="block">↑ GOING OUT</span><small>{outgoing.length} · {formatMoney(summary.outgoingAgreed)}</small></button></div>
          {eligiblePercentageItems.length ? <div className="flex gap-1 overflow-x-auto pb-0.5 lg:hidden" aria-label={`${selectedSide} percentage presets`}>{(selectedSide === "incoming" ? incomingDealPercentages : outgoingDealPercentages).map((percentage) => <button type="button" key={percentage} onClick={() => applyPercentageToSide(percentage)} className="min-h-9 min-w-11 rounded-lg bg-slate-100 px-2 text-xs font-black text-slate-700 dark:bg-slate-800 dark:text-slate-200">{percentage}</button>)}<button type="button" onClick={() => { const value = window.prompt("Custom percentage", customPercentage); if (value != null && Number.isFinite(Number(value))) { setCustomPercentage(value); applyPercentageToSide(Number(value)); } }} className="min-h-9 rounded-lg border border-dashed border-slate-300 px-2 text-xs font-black text-slate-500">Custom</button></div> : null}
          <div className="grid grid-cols-3 rounded-xl bg-slate-950 px-2 py-2 text-white lg:hidden"><span className="text-[10px] text-slate-400">IN<b className="ml-1 text-xs text-white">{formatMoney(summary.incomingAgreed + transaction.cashReceived)}</b></span><span className="text-center text-[10px] text-slate-400">OUT<b className="ml-1 text-xs text-white">{formatMoney(summary.outgoingAgreed + transaction.cashPaid)}</b></span><span className="text-right text-[10px] text-slate-400">DIFFERENCE<b className={`ml-1 text-xs ${summary.cashDifference < 0 ? "text-orange-300" : "text-emerald-300"}`}>{formatMoney(summary.cashDifference)}</b></span>{compactMetricLabel ? <span className="col-span-3 mt-1 border-t border-white/10 pt-1 text-center text-[10px] text-slate-400">{compactMetricLabel} <b className={compactMetric == null ? "text-amber-300" : compactMetric >= 0 ? "text-emerald-300" : "text-rose-300"}>{compactMetric == null ? "Pending basis" : formatMoney(compactMetric)}</b></span> : null}</div>
        </div>
        <div className="lg:hidden">{sidePanel(selectedSide)}</div>
        <div className="hidden gap-3 lg:grid lg:grid-cols-2">{sidePanel("incoming")}{sidePanel("outgoing")}</div>
        <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-night-900"><button type="button" onClick={() => setPaymentsOpen(true)} className="flex min-h-12 w-full items-center gap-3 px-3 text-left"><WalletCards size={17} className="text-violet-500" /><span className="min-w-0 flex-1"><b className="block text-sm">Payment</b><small className="block truncate text-slate-500">{paymentSummary}</small></span><ChevronRight size={17} className="text-slate-400" /></button><button type="button" onClick={() => setPhotosOpen(true)} className="flex min-h-12 w-full items-center gap-3 border-t border-slate-100 px-3 text-left dark:border-slate-800"><ImagePlus size={17} className="text-sky-500" /><span className="min-w-0 flex-1"><b className="block text-sm">Photos</b><span className="mt-1 flex gap-1">{images.slice(0, 3).map((image) => <img key={image.id} src={image.imageUrl} alt="" className="size-6 rounded object-cover" />)}</span></span><small className="font-black text-slate-500">{images.length} / {TRANSACTION_PHOTO_LIMIT}</small><ChevronRight size={17} className="text-slate-400" /></button>{transaction.notes ? <button type="button" onClick={() => setDetailsOpen(true)} className="flex min-h-12 w-full items-center gap-3 border-t border-slate-100 px-3 text-left dark:border-slate-800"><CircleDollarSign size={17} className="text-amber-500" /><span className="min-w-0 flex-1"><b className="block text-sm">Notes</b><small className="block truncate text-slate-500">{transaction.notes}</small></span><ChevronRight size={17} className="text-slate-400" /></button> : null}</section>
      </main>
    ...
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-3 pt-2 pb-[calc(.5rem+env(safe-area-inset-bottom))] shadow-[0_-12px_30px_rgba(15,23,42,.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-950/95 lg:left-64"><div className="mx-auto flex max-w-5xl items-center gap-2"><button type="button" onClick={() => step === "review" ? setStep("build") : navigate("/sales")} disabled={busy || imageUploading} aria-label="Back" className="hidden size-11 shrink-0 place-items-center rounded-xl bg-slate-100 disabled:opacity-50 dark:bg-slate-800 sm:grid"><ArrowLeft size={18} /></button><button type="button" onClick={() => void savePending()} disabled={busy || imageUploading || classification === "unclassified"} className="inline-flex min-h-11 flex-[0.8] items-center justify-center gap-2 rounded-xl bg-amber-100 px-3 text-sm font-black text-amber-900 disabled:opacity-40"><Save size={16} /> Pending</button>{step === "build" ? <button type="button" onClick={() => { setMessage(""); setStep("review"); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={classification === "unclassified" || busy || imageUploading} className="inline-flex min-h-11 flex-[1.2] items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 text-sm font-black text-white disabled:opacity-40">Review Deal <ArrowRight size={17} /></button> : <button type="button" onClick={() => void completeDeal()} disabled={busy || imageUploading} className="inline-flex min-h-11 flex-[1.2] items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-black text-white disabled:opacity-40"><Check size={17} /> Confirm {typeLabel(classification)}</button>}</div></div>

    <ResponsiveModal open={detailsOpen} title="Deal Details" description="Date, customer, event, and notes stay available without taking space on the deal builder." onClose={() => setDetailsOpen(false)} size="md"><div className="space-y-3"><label><span className="text-xs font-black">Date & time</span><input type="datetime-local" value={transaction.tradeDate.slice(0, 16)} onChange={(event) => updateTransaction({ ...transaction, tradeDate: event.target.value })} className={`${inputClass} mt-1`} /></label><label><span className="text-xs font-black">Customer / seller</span><input value={transaction.tradePartner || ""} onChange={(event) => updateTransaction({ ...transaction, tradePartner: event.target.value })} placeholder="Optional name" className={`${inputClass} mt-1`} /></label><label><span className="text-xs font-black">Event</span><select value={transaction.eventId || ""} onChange={(event) => updateTransaction({ ...transaction, eventId: event.target.value || undefined })} className={`${inputClass} mt-1`}><option value="">No event</option>{events.map((event) => <option key={event.id} value={event.id}>{event.name}</option>)}</select></label><label><span className="text-xs font-black">Notes</span><textarea value={transaction.notes || ""} onChange={(event) => updateTransaction({ ...transaction, notes: event.target.value })} rows={4} placeholder="Condition notes, bundle terms, customer details…" className={`${inputClass} mt-1`} /></label><AppButton onClick={() => setDetailsOpen(false)} className="w-full"><Check size={17} /> Done</AppButton></div></ResponsiveModal>

    <ResponsiveModal open={paymentsOpen} title="Payment" description="Add received or paid amounts across one or several payment methods." onClose={() => setPaymentsOpen(false)} size="lg"><div className="space-y-3">{payments.map((payment) => <div key={payment.id} className="rounded-xl border border-slate-200 p-3 dark:border-slate-800"><div className="grid grid-cols-2 gap-2"><select value={payment.direction} onChange={(event) => changePayments(payments.map((row) => row.id === payment.id ? { ...row, direction: event.target.value as TransactionPaymentEntry["direction"] } : row))} className={inputClass}><option value="received">Received</option><option value="paid">Paid</option></select><select value={payment.paymentMethod} onChange={(event) => changePayments(payments.map((row) => row.id === payment.id ? { ...row, paymentMethod: event.target.value as SalePaymentMethod } : row))} className={inputClass}>{paymentMethods.map((method) => <option key={method} value={method}>{paymentMethodLabels[method]}</option>)}</select><input type="number" min="0" step=".01" value={payment.amount || ""} placeholder="$0" onChange={(event) => changePayments(payments.map((row) => row.id === payment.id ? { ...row, amount: Number(event.target.value || 0) } : row))} className={inputClass} /><button type="button" onClick={() => changePayments(payments.filter((row) => row.id !== payment.id))} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-rose-50 px-3 font-black text-rose-600"><Trash2 size={16} /> Remove</button>{payment.direction === "paid" ? <select value={payment.paidByWorkerId || ""} onChange={(event) => changePayments(payments.map((row) => row.id === payment.id ? { ...row, paidByWorkerId: event.target.value || undefined } : row))} className="col-span-2"><option value="">Who paid: Unassigned</option>{workers.map((worker) => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select> : null}</div></div>)}{!payments.length ? <p className="rounded-xl bg-slate-50 p-4 text-center text-sm font-bold text-slate-500 dark:bg-slate-900">No payment added.</p> : null}<div className="grid grid-cols-2 gap-2"><AppButton variant="secondary" onClick={() => addPayment("received")}><Plus size={16} /> Received</AppButton><AppButton variant="secondary" onClick={() => addPayment("paid")}><Plus size={16} /> Paid</AppButton></div><AppButton onClick={() => setPaymentsOpen(false)} className="w-full"><Check size={17} /> Done</AppButton></div></ResponsiveModal>
    ...
    <ResponsiveModal open={Boolean(editing) && !bulkReviewId} title={editing?.itemName || "Deal Item"} description="Market, condition, percentage, agreed value, cost basis, ownership, and item photos remain separate." onClose={() => { setEditing(undefined); setManualSearch(false); setScanFile(undefined); }} size="lg" dismissible={!imageUploading} swipeToDismiss={false} closeOnBackdrop={false} mobileEditor>
      {editing ? <div className="space-y-4">
        ...
        <section className="rounded-2xl border border-slate-200 p-3 dark:border-slate-800"><div className="grid gap-3 sm:grid-cols-3"><label><span className="text-xs font-black">Confirmed market</span><input type="number" min="0" step=".01" value={editing.marketValue || ""} onChange={(event) => updateItem({ ...editing, marketValue: Number(event.target.value || 0) })} className={`${inputClass} mt-1`} /></label><label><span className="text-xs font-black">Condition</span><select value={editing.cardCondition || "Near Mint / NM"} onChange={(event) => { const condition = event.target.value as CardCondition; const providerMarket = editing.tcgplayerPricing?.variants.find((variant) => variant.variant === editing.tcgplayerPricing?.selectedVariant)?.market; updateItem({ ...editing, cardCondition: condition, marketValue: condition === "Near Mint / NM" ? providerMarket ?? 0 : 0 }); }} className={`${inputClass} mt-1`}>{conditions.map((condition) => <option key={condition} value={condition}>{condition}</option>)}</select></label><div className="rounded-xl bg-slate-100 p-3 dark:bg-slate-900"><small className="text-slate-500">Market used for percentage</small><b className="mt-1 block text-lg">{formatMoney(conditionAdjustedMarket(editing.marketValue, editing.cardCondition))}</b></div></div><p className="mt-2 text-xs text-slate-500">NM may use the selected provider market. LP, MP, HP, and Damaged require your manually confirmed market value; no condition percentage is assumed.</p><div className="mt-3 flex flex-wrap gap-2">{(editing.direction === "incoming" ? incomingDealPercentages : outgoingDealPercentages).map((percentage) => <button type="button" key={percentage} onClick={() => updateItem(applyDealPercentage(editing, editing.direction as DealSide, percentage))} className={`min-h-10 rounded-xl px-3 text-sm font-black ${editing.targetBuyPercentage === percentage ? "bg-violet-600 text-white" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}>{percentage}%</button>)}<span className="inline-flex min-h-10 items-center rounded-xl border border-dashed border-slate-300 px-3 text-xs font-bold text-slate-500">Custom via agreed value</span></div><div className="mt-3 grid gap-3 sm:grid-cols-2"><label><span className="text-xs font-black">Percentage</span><input type="number" min="0" max="200" step=".1" value={editing.targetBuyPercentage ?? ""} onChange={(event) => updateItem(applyDealPercentage(editing, editing.direction as DealSide, Number(event.target.value || 0)))} className={`${inputClass} mt-1`} /></label><label><span className="text-xs font-black">Final agreed value</span><input type="number" min="0" step=".01" value={editing.agreedTradeValue || ""} onChange={(event) => { const agreed = Number(event.target.value || 0); updateItem(editing.direction === "incoming" ? { ...editing, agreedTradeValue: agreed, boughtPrice: agreed, costBasis: agreed } : { ...editing, agreedTradeValue: agreed, soldPrice: agreed }); }} className={`${inputClass} mt-1`} /></label></div></section>
        ...
        <button type="button" onClick={() => { updateTransaction({ ...transaction, items: transaction.items.filter((item) => item.id !== editing.id) }); setEditing(undefined); }} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-rose-50 px-3 text-sm font-black text-rose-700 dark:bg-rose-950/30 dark:text-rose-200"><Trash2 size={16} /> Remove item</button>
