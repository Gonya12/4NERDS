    ...
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-3 gap-3">
        <div className="rounded-2xl bg-white/90 p-4 shadow-soft">
          ...
          <p className="mt-3 text-2xl font-black text-ink">{upcoming.length}</p>
          <p className="text-xs text-slate-500">Upcoming</p>
        </div>
        <div className="rounded-2xl bg-white/90 p-4 shadow-soft">
          <Users className="text-emerald-600" size={20} />
          <p className="mt-3 text-2xl font-black text-ink">{confirmedCount}</p>
      ...
      {highlighted.length > 0 ? (
        <section className="rounded-2xl border border-orange-100 bg-orange-50 p-4">
          <h2 className="font-black text-ink">Coming Up Soon</h2>
      ...
      <section>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="text-xl font-black text-ink">Upcoming Events</h2>
          <Link to="/events" className="text-sm font-bold text-coral">View all</Link>
        </div>
        {upcoming.length === 0 ? (
          <EmptyState title="No events yet. Add your first vendor event." action={<Link to="/events/new" className="rounded-lg bg-ink px-4 py-3 text-sm font-bold text-white">Add Event</Link>} />
        ) : (
